...
js 有五个迭代方法,分别为:**every**、**some**、**filter**、**map**、**forEach**
...
- vue-router 核心实现原理：(更新视图但不重新请求页面，是前端路由原理的核心之一);目前在浏览器环境中这一功能的实现主要有 2 种方式,Hash 模式和 History 模式：
  ...
  3. 解析配置的路由，即解析 router 的配置项 routes，能根据路由匹配到对应组件；

  4. 实现两个全局组件 router-link 和 router-view；（最终落地点）
...
- 总结来说:vue3 完善了自己的工具链，做了更好的工程化，将 webpack 的配置整合到自己的体系中去，但同时也让 vue 逐渐拥有了壁垒
...
- 第一种、使用全局函数 beforeEach,直接来获取 form.path(即为对应的上一次地址的路由 path 内容)；
...
[Sentry前端部署拓展篇（sourcemap关联、issue关联、release控制）](https://segmentfault.com/a/1190000014683598#articleHeader5)
...
2. 后端处理 成功 返回 用户详情(用户名,头像,菜单权限(一般后台系统会用到),登录地点 时间 登录平台 IP地址(一般后台系统会用到) 等等) 和 jwt token

3. 前端拿到 把jwt token 存到 storge 或者 cookie 里面

4. 前端 刷新页面 或者 做其他操作 在 router.beforeEach 里面 请求 用户 信息

5. 请求用户信息时 看是否存在 token ;存在 token 发过去 看返回信息;不存在 提示 token过期 并且 重定向到 登录页面

6. 后端 验证 前端传过来的 token 是否正确(过期,被篡改 等等),验证失败 401 ,前端 收到401 提示 token 过期    并且 重定向到 登录页面

7. 后端 验证 成功 ;回到第 2 步

- 在 第 2 步里,后端处理时 会根据用户名 找到 用户id 一般后端会往数据库里 存这个 id 和 登录时间 等等 

- 第2步里,后端 每次生成token 时建议 先存一个时间戳 然后根据这个生成 token  ,每次生成token 时,上一次生成的 失效.

- 前端 可以把 token 存到 cookie里面 或者 存到 localStorage 里面. 与后端通信时,可以放到 cookie里面 自动发送(不能跨域),或者放到HTTP 请求的头信息Authorization字段里面。或者 放到 post请求数据体里面(axios 请求拦截器 判断请求是否 是post,是的话 把token 放到请求体data里面)

- 实现单点登录 

- 大厂的秘钥是随机生成放到共享缓存里的

- 大厂都是通过在统一的cas服务完成jwt检验，而不是微服务自己完成

[web项目中用户登录的具体流程session](https://www.cnblogs.com/smilond/p/10476909.html)
...
husky能做的这个也能做 (git hook)

[husky](https://github.com/typicode/husky#readme)

在commit 和 push 前做些什么(git hook)
